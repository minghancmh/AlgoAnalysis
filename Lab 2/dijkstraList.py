...
class Graph():
    def __init__(self, V):
        self.V = V
        self.adj = [[] for vertexPair in range(V)] #initialise adjacency list
    ...
    def shortestPath(self, src):
        pq = [] # heap contains tuples in the format (dist, vertex)
        ...
        while pq:
            ...
            for v, weight in self.adj[u]: # look at the vertices that are connected to u
                if dist[v] > dist[u] + weight:
                    ...
                    heapq.heappush(pq, (dist[v],v)) # node v has been added to the solution set, we push it to the heap
